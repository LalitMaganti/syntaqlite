...
pub struct CatalogLayerContents {
    ...
    /// Lowercased relation names this layer masks from outer layers — a
    /// tombstone for `ALTER TABLE ... RENAME TO` / `DROP TABLE`. A name found
    /// here during innermost-first resolution reads as absent even if an outer
    /// layer still defines it.
    removed_relations: HashSet<String>,
}
...
impl Catalog {
    ...
    pub(crate) fn accumulate_ddl(
        ...
        match role {
            SemanticRole::DefineTable {
                ...
                self.layers[target.index()].insert_table(
                    name_val.to_string(),
                    cols,
                    is_without_rowid,
                );
            ...
            SemanticRole::DefineView {
                ...
                let cols = reader.extract_columns(&fields, columns, select);
                self.layers[target.index()].insert_view(name_val.to_string(), cols);
            }
    ...
    fn accumulate_alter_table(
        ...
        let Some(mut entry) = self.visible_relation(&table_name).cloned() else {
            return;
        };
    ...
    /// either tombstones `name` (→ `None`) or defines it (→ its entry) wins.
    fn visible_relation(&self, name: &str) -> Option<&RelationEntry> {
        let key = name.to_ascii_lowercase();
        for layer in self.all_layers_ordered() {
            if layer.removed_relations.contains(&key) {
                return None;
            }
            if let Some(rel) = layer.relations.get(&key) {
                return Some(rel);
            }
        }
        None
    ...
    pub(crate) fn resolve_relation(&self, name: &str) -> bool {
        self.visible_relation(name).is_some()
    }

    /// Returns `true` if `name` is (or was) defined as a relation in any layer,
    ...
    pub(crate) fn relation_defined(&self, name: &str) -> bool {
        let key = name.to_ascii_lowercase();
        self.layers
            .iter()
    ...
    pub(crate) fn is_view(&self, name: &str) -> bool {
        self.visible_relation(name).is_some_and(|r| r.is_view)
    }
    ...
    pub(crate) fn all_relation_names(&self) -> Vec<String> {
        ...
        names.retain(|n| self.resolve_relation(n));
        names
    }
...
fn alter_qualified_name(
    ...
    let (text, _) = stmt.name_text(Some(obj_id));
    (!text.is_empty()).then(|| text.to_string())
}

/// Identifier text of an `ALTER` name field (a `Name` node), or `None`.
fn alter_name<'s>(stmt: &AnyParsedStatement<'s>, fields: &NodeFields, idx: u8) -> Option<&'s str> {
    let id = fields.node_id_at(idx)?;
    let (text, _) = stmt.name_text(Some(id));
...
mod tests {
    ...
    fn alter_add_column_none_columns_stays_none() {
        ...
        assert_eq!(cat.table_source_info("x1").0, None);
    }
    ...
    fn alter_of_unknown_table_is_noop() {
        ...
        assert!(!cat.resolve_relation("nosuch"));
    }
    ...
    fn alter_rename_table_masks_old_and_registers_new_across_promotion() {
        ...
        assert!(cat.resolve_relation("t5"));
        assert!(!cat.resolve_relation("t3"));
        // ...but the rename's own existence check still sees the target
        // (tombstone-ignoring), so RENAME TO doesn't flag its own source.
        assert!(cat.relation_defined("t3"));
        assert_eq!(cat.table_source_info("t5").0, Some(vec!["a".to_string()]));
    ...
    fn recreating_a_renamed_table_clears_the_tombstone() {
        ...
        assert!(cat.resolve_relation("t3"));
        assert!(cat.resolve_relation("t5"));
        assert_eq!(cat.table_source_info("t3").0, Some(vec!["b".to_string()]));
    }
