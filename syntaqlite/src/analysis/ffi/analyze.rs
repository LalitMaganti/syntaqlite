...
use super::{
    AnalyzerState, PerStatementCache, SyntaqliteAnalyzer, SyntaqliteColumnLineage,
    SyntaqliteColumnOrigin, SyntaqliteDiagnostic, SyntaqlitePhysicalTableAccess,
...
pub unsafe extern "C" fn syntaqlite_analyzer_analyze(
    ...
    let mut ctx = AnalysisContext::new(&mut state.user_catalog).with_config(state.analysis_config);
...
pub(super) fn populate_lineage(state: &mut AnalyzerState, model: &Analysis) {
