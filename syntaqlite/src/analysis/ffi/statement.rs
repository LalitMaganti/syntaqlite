...
use crate::analysis::lineage::RelationKind;
use crate::analysis::model::StatementAnalysis;

use super::codes::{diagnostic_code_to_c, severity_to_c};
use super::{
    PerStatementCache, SyntaqliteAnalyzer, SyntaqliteColumnLineage, SyntaqliteColumnOrigin,
    SyntaqliteDefinedRelation, SyntaqliteDiagnostic, SyntaqlitePhysicalTableAccess,
    SyntaqliteRelationAccess, SyntaqliteUnexpandedView,
};

fn ensure_source<'a>(cache: &'a mut PerStatementCache, stmt: &StatementAnalysis) -> &'a CString {
    cache
        .source
...
unsafe fn stmt_cache<'a>(
    ...
    idx: u32,
) -> Option<(&'a StatementAnalysis, &'a mut PerStatementCache)> {
    ...
    let stmt: &StatementAnalysis = unsafe { &*std::ptr::from_ref::<StatementAnalysis>(stmt) };
    Some((stmt, cache))
}
...
/// `v` must be a valid pointer from `syntaqlite_analyzer_create_*`.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn syntaqlite_analyzer_statement_source(
    v: *mut SyntaqliteAnalyzer,
    idx: u32,
) -> *const c_char {
...
#[unsafe(no_mangle)]
#[expect(clippy::cast_possible_truncation)]
pub unsafe extern "C" fn syntaqlite_analyzer_statement_count(v: *mut SyntaqliteAnalyzer) -> u32 {
    // SAFETY: caller guarantees `v` is a valid pointer from `syntaqlite_analyzer_create_*`.
    let state = unsafe { &mut *v }.state_mut();
    state
        .last_model
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn syntaqlite_analyzer_statement_diagnostic_count(
    v: *mut SyntaqliteAnalyzer,
    idx: u32,
) -> u32 {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn syntaqlite_analyzer_statement_diagnostics(
    v: *mut SyntaqliteAnalyzer,
    idx: u32,
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn syntaqlite_analyzer_statement_column_lineage_count(
    v: *mut SyntaqliteAnalyzer,
    idx: u32,
) -> u32 {
    // SAFETY: caller guarantees `v` is valid.
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn syntaqlite_analyzer_statement_column_lineage(
    v: *mut SyntaqliteAnalyzer,
    idx: u32,
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn syntaqlite_analyzer_statement_relation_count(
    v: *mut SyntaqliteAnalyzer,
    idx: u32,
) -> u32 {
    // SAFETY: caller guarantees `v` is valid.
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn syntaqlite_analyzer_statement_relations(
    v: *mut SyntaqliteAnalyzer,
    idx: u32,
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn syntaqlite_analyzer_statement_physical_table_count(
    v: *mut SyntaqliteAnalyzer,
    idx: u32,
) -> u32 {
    // SAFETY: caller guarantees `v` is valid.
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn syntaqlite_analyzer_statement_physical_tables(
    v: *mut SyntaqliteAnalyzer,
    idx: u32,
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn syntaqlite_analyzer_statement_defined_relation_count(
    v: *mut SyntaqliteAnalyzer,
    idx: u32,
) -> u32 {
    // SAFETY: caller guarantees `v` is valid.
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn syntaqlite_analyzer_statement_defined_relations(
    v: *mut SyntaqliteAnalyzer,
    idx: u32,
...
/// `v` must be a valid pointer from `syntaqlite_analyzer_create_*`.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn syntaqlite_analyzer_statement_unexpanded_view_count(
    v: *mut SyntaqliteAnalyzer,
    idx: u32,
) -> u32 {
    // SAFETY: caller guarantees `v` is valid.
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn syntaqlite_analyzer_statement_unexpanded_views(
    v: *mut SyntaqliteAnalyzer,
    idx: u32,
