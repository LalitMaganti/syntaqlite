...
use super::{AnalyzerState, SyntaqliteAnalyzer};

pub(super) fn create_validator(dialect: AnyDialect) -> *mut SyntaqliteAnalyzer {
    let analyzer = Analyzer::with_dialect(dialect.clone());
    let user_catalog = Catalog::new(dialect.clone());

    let state = Box::new(AnalyzerState {
        analyzer,
        user_catalog,
        dialect,
    ...
    Box::into_raw(state).cast::<SyntaqliteAnalyzer>()
}
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn syntaqlite_analyzer_create_with_dialect(
    dialect: crate::dialect::ffi::CDialect,
) -> *mut SyntaqliteAnalyzer {
...
#[cfg(feature = "sqlite")]
#[unsafe(no_mangle)]
pub extern "C" fn syntaqlite_analyzer_create_sqlite() -> *mut SyntaqliteAnalyzer {
...
/// # Safety
///
/// `v` must be NULL or a valid pointer from `syntaqlite_analyzer_create_sqlite`.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn syntaqlite_analyzer_destroy(v: *mut SyntaqliteAnalyzer) {
    if !v.is_null() {
        // SAFETY: `v` was created by `Box::into_raw` in `create_validator`.
        drop(unsafe { Box::from_raw(v.cast::<AnalyzerState>()) });
    }
}
