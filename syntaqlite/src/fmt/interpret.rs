            ...
            match op {
                FmtOp::Keyword(sid) => {
                    ...
                    if let Some(ref cctx) = ctx.comment_ctx {
                        let drain = cctx.take_keyword(kw_text, source, arena);
                ...
                FmtOp::Span(idx) => {
                    ...
                    if quoted || !s.is_empty() {
                        ...
                        if quoted {
                            ...
                            let open = arena.text(open);
                            let close = arena.text(close);
                            running = arena.cat(running, open);
                            running = arena.cat(running, txt);
                            running = arena.cat(running, close);
                        } else {
                            let txt = arena.text(s);
                            running = arena.cat(running, txt);
                        }
