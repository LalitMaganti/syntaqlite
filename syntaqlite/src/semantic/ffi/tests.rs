...
// `*mut SyntaqliteValidator` is obtained from
// `syntaqlite_validator_create_sqlite` (non-null, valid for the guard's
...
struct Validator(*mut SyntaqliteValidator);
...
impl Validator {
    ...
    fn as_ptr(&self) -> *mut SyntaqliteValidator {
        self.0
    }

    /// Analyze SQL and return the diagnostic count.
    fn analyze(&self, sql: &str) -> u32 {
        // SAFETY: handle from `syntaqlite_validator_create_sqlite`; `sql` outlives the call.
        unsafe {
            syntaqlite_validator_analyze(
                self.0,
                sql.as_ptr().cast(),
    ...
    fn diag_count(&self) -> u32 {
        // SAFETY: handle from `syntaqlite_validator_create_sqlite`.
        unsafe { syntaqlite_validator_diagnostic_count(self.0) }
    }

    /// Raw pointer to the aggregate diagnostics buffer (may be null).
    fn diags_ptr(&self) -> *const SyntaqliteDiagnostic {
        // SAFETY: handle from `syntaqlite_validator_create_sqlite`.
        unsafe { syntaqlite_validator_diagnostics(self.0) }
    }
    ...
    fn diag_msg(&self, i: usize) -> String {
        ...
        unsafe {
            let ptr = syntaqlite_validator_diagnostics(self.0);
            assert!(!ptr.is_null());
    ...
    fn has_code(&self, code: u32) -> bool {
        // SAFETY: handle from `syntaqlite_validator_create_sqlite`.
        unsafe {
            let n = syntaqlite_validator_diagnostic_count(self.0) as usize;
            let ptr = syntaqlite_validator_diagnostics(self.0);
    ...
    fn render(&self, file: Option<&CStr>) -> String {
        // SAFETY: handle from `syntaqlite_validator_create_sqlite`; `file`'s CStr outlives the call.
        unsafe {
            let file_ptr = file.map_or(std::ptr::null(), CStr::as_ptr);
            let ptr = syntaqlite_validator_render_diagnostics(self.0, file_ptr);
            assert!(!ptr.is_null());
    ...
    fn set_check_level(&self, name: &str, level: u32) -> i32 {
        ...
        // SAFETY: handle from `syntaqlite_validator_create_sqlite`; `c_name` outlives the call.
        unsafe { syntaqlite_validator_set_check_level(self.0, c_name.as_ptr(), level) }
    }
    ...
    fn add_relation(&self, name: &str, cols: &[&str], is_view: bool) {
        ...
        // SAFETY: FFI test — handle from `syntaqlite_validator_create_sqlite`;
        // all C strings and the `SyntaqliteRelationDef` outlive the call.
        unsafe {
            if is_view {
                syntaqlite_validator_add_views(self.0, &raw const rel, 1);
            } else {
                syntaqlite_validator_add_tables(self.0, &raw const rel, 1);
            }
        }
    }
    ...
    fn load_schema_ddl(&self, ddl: &str) -> u32 {
        ...
        unsafe {
            syntaqlite_validator_load_schema_ddl(self.0, ddl.as_ptr().cast(), ddl.len() as u32)
        }
    }
...
/// the handle comes from `syntaqlite_validator_create_sqlite` and is
        ...
        impl Validator {
            $(
                fn $const_name(&self) -> $const_ret {
                    // SAFETY: `self.0` from `syntaqlite_validator_create_sqlite`;
...
forward! {
    const
        ,lineage_complete -> u32 = syntaqlite_validator_lineage_complete
        ,column_lineage_count -> u32 = syntaqlite_validator_column_lineage_count
        ,column_lineage -> *const SyntaqliteColumnLineage = syntaqlite_validator_column_lineage
        ,relation_count -> u32 = syntaqlite_validator_relation_count
        ,relations -> *const SyntaqliteRelationAccess = syntaqlite_validator_relations
        ,physical_table_count -> u32 = syntaqlite_validator_physical_table_count
        ,physical_tables -> *const SyntaqlitePhysicalTableAccess = syntaqlite_validator_physical_tables
    ;
    mut
        ,set_mode(mode: u32) -> () = syntaqlite_validator_set_mode
        ,set_strict_schema(enabled: u32) -> () = syntaqlite_validator_set_strict_schema
        ,set_suggestion_threshold(t: u32) -> () = syntaqlite_validator_set_suggestion_threshold
        ,reset_catalog() -> () = syntaqlite_validator_reset_catalog
        ,stmt_count() -> u32 = syntaqlite_validator_statement_count
        ,stmt_diag_count(i: u32) -> u32 = syntaqlite_validator_statement_diagnostic_count
        ,stmt_diags(i: u32) -> *const SyntaqliteDiagnostic = syntaqlite_validator_statement_diagnostics
        ,stmt_lineage_count(i: u32) -> u32 = syntaqlite_validator_statement_column_lineage_count
        ,stmt_lineage(i: u32) -> *const SyntaqliteColumnLineage = syntaqlite_validator_statement_column_lineage
        ,stmt_relation_count(i: u32) -> u32 = syntaqlite_validator_statement_relation_count
        ,stmt_relations(i: u32) -> *const SyntaqliteRelationAccess = syntaqlite_validator_statement_relations
        ,stmt_physical_table_count(i: u32) -> u32 = syntaqlite_validator_statement_physical_table_count
        ,stmt_physical_tables(i: u32) -> *const SyntaqlitePhysicalTableAccess = syntaqlite_validator_statement_physical_tables
        ,stmt_defined_count(i: u32) -> u32 = syntaqlite_validator_statement_defined_relation_count
        ,stmt_defined(i: u32) -> *const SyntaqliteDefinedRelation = syntaqlite_validator_statement_defined_relations
        ,stmt_source(i: u32) -> *const c_char = syntaqlite_validator_statement_source
        ,stmt_unexpanded_view_count(i: u32) -> u32 = syntaqlite_validator_statement_unexpanded_view_count
        ,stmt_unexpanded_views(i: u32) -> *const SyntaqliteUnexpandedView = syntaqlite_validator_statement_unexpanded_views
    ;
}
...
impl Drop for Validator {
    fn drop(&mut self) {
        if !self.0.is_null() {
            ...
            unsafe { syntaqlite_validator_destroy(self.0) };
        }
    }
}
...
fn set_module_resolver_callback_no_crash() {
    ...
    let set = |cb| unsafe {
        syntaqlite_validator_set_module_resolver(v.as_ptr(), cb, std::ptr::null_mut());
    };
