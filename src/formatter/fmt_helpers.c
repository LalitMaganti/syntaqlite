...
extern inline uint32_t synq_span_text(SynqFmtCtx* ctx,
                                      SyntaqliteSourceSpan span);
extern inline uint32_t synq_emit_single_comment(SynqFmtCtx* ctx,
                                                uint32_t tok_idx);
...
static CommentMap* comment_map_build(const char* source,
                                     SynqTokenList* token_list) {
  if (!token_list || token_list->count == 0)
    return NULL;
...
static uint32_t find_preceding_node(SynqAstContext* astCtx, uint32_t offset) {
...
SynqCommentAttachment* synq_comment_attach(SynqAstContext* astCtx,
                                           uint32_t root_id,
                                           const char* source,
                                           SynqTokenList* token_list) {
  if (!token_list || token_list->count == 0)
...
uint32_t synq_emit_owned_comments(SynqFmtCtx* ctx,
                                  uint32_t node_id,
                                  uint8_t kind) {
  if (!ctx->comment_att)
  ...
  for (uint32_t i = 0; i < ctx->comment_att->count && n < 62; i++) {
    if (ctx->comment_att->owner_node[i] != node_id)
      continue;
    if (ctx->comment_att->position[i] != kind)
      continue;
    ...
    if (kind == SYNQ_COMMENT_LEADING) {
      ...
      if (n < 62)
        parts[n++] = is_line ? synq_doc_hardline(&ctx->docs)
      ...
      if (is_line) {
        ...
        if (n < 62)
          parts[n++] = synq_doc_break_parent(&ctx->docs);
      } else {
...
uint32_t synq_format_clause(SynqFmtCtx* ctx,
                            const char* keyword,
                            uint32_t body_id) {
  if (body_id == SYNTAQLITE_NULL_NODE)
    return SYNQ_NULL_DOC;
  // Extract body's leading comments so they appear before the keyword.
  uint32_t leading =
      synq_emit_owned_comments(ctx, body_id, SYNQ_COMMENT_LEADING);
  uint32_t kw_doc = synq_kw(ctx, keyword);
  uint32_t body_doc = synq_fmt_interpret(ctx, body_id);
  uint32_t trailing =
      synq_emit_owned_comments(ctx, body_id, SYNQ_COMMENT_TRAILING);
  if (body_doc == SYNQ_NULL_DOC)
    return SYNQ_NULL_DOC;
  ...
  uint32_t items[] = {
      ...
      leading,
      kw_doc,
      synq_doc_group(&ctx->docs,
                     synq_doc_nest(&ctx->docs,
                                   (int32_t)ctx->options->indent_width, inner)),
  };
...
uint32_t synq_format_node(SynqFmtCtx* ctx, uint32_t node_id) {
  ...
  uint32_t leading =
      synq_emit_owned_comments(ctx, node_id, SYNQ_COMMENT_LEADING);
  uint32_t body = synq_fmt_interpret(ctx, node_id);
  uint32_t trailing =
      synq_emit_owned_comments(ctx, node_id, SYNQ_COMMENT_TRAILING);
