...
static uint32_t interpret_op(SynqFmtCtx* ctx,
                             const void* node_ptr,
                             const SynqFmtOp* ops,
                             uint32_t* pos,
...
static uint32_t interpret_op(SynqFmtCtx* ctx,
  ...
  switch (op->kind) {
    ...
    case SYNQ_FOP_NEST: {
      uint32_t child = interpret_op(ctx, node_ptr, ops, pos, loop_child_id);
      return synq_doc_nest(&ctx->docs, (int32_t)ctx->options->indent_width,
                           child);
    }
    ...
    case SYNQ_FOP_IF_SET: {
      ...
      if (field_val != SYNTAQLITE_NULL_NODE) {
        uint32_t then_doc =
            interpret_op(ctx, node_ptr, ops, pos, loop_child_id);
        if (op->flags & SYNQ_FOP_HAS_ELSE)
          skip_op(ops, pos);
        return then_doc;
      }
    ...
    case SYNQ_FOP_IF_FLAG: {
      ...
      bool test = op->enum_value ? (raw & (uint8_t)op->enum_value) : (raw != 0);
      if (test) {
        uint32_t then_doc =
            interpret_op(ctx, node_ptr, ops, pos, loop_child_id);
        if (op->flags & SYNQ_FOP_HAS_ELSE)
    ...
    case SYNQ_FOP_SWITCH: {
      ...
      for (uint16_t i = 0; i < op->count; i++) {
        ...
        if (!matched && (int)case_op->enum_value == val) {
          result = interpret_op(ctx, node_ptr, ops, pos, loop_child_id);
          matched = true;
        } else {
          skip_op(ops, pos);
        }
    ...
    case SYNQ_FOP_FOR_EACH: {
      ...
      if (has_sep)
        skip_op(ops, &end_pos);  // end_pos = past separator
      ...
      for (uint32_t i = 0; i < count; i++) {
        if (i > 0 && has_sep) {
          ...
          buf[n++] = interpret_op(ctx, node_ptr, ops, &sp, children[i]);
        }
