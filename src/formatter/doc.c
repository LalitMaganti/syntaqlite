...
uint32_t synq_doc_text(SynqDocContext* ctx, const char* text, uint32_t length) {
  uint32_t id =
      synq_arena_alloc(&ctx->arena, SYNQ_DOC_TEXT, sizeof(SynqDocText));
  SynqDocText* node =
      &((SynqDoc*)(ctx->arena.data + ctx->arena.offsets[id]))->text;
  node->length = length;
  node->text = text;
  return id;
}
...
uint32_t synq_doc_nest(SynqDocContext* ctx, int32_t indent, uint32_t child) {
  uint32_t id =
      synq_arena_alloc(&ctx->arena, SYNQ_DOC_NEST, sizeof(SynqDocNest));
  SynqDocNest* node =
      &((SynqDoc*)(ctx->arena.data + ctx->arena.offsets[id]))->nest;
  node->indent = indent;
  node->child = child;
  return id;
}
...
uint32_t synq_doc_group(SynqDocContext* ctx, uint32_t child) {
  uint32_t id =
      synq_arena_alloc(&ctx->arena, SYNQ_DOC_GROUP, sizeof(SynqDocGroup));
  SynqDocGroup* node =
      &((SynqDoc*)(ctx->arena.data + ctx->arena.offsets[id]))->group;
  node->child = child;
  return id;
}

uint32_t synq_doc_concat(SynqDocContext* ctx,
                         uint32_t* children,
                         uint32_t count) {
  ...
  SynqDocConcat* node =
      &((SynqDoc*)(ctx->arena.data + ctx->arena.offsets[id]))->concat;
...
uint32_t synq_doc_line_suffix(SynqDocContext* ctx, uint32_t child) {
  uint32_t id = synq_arena_alloc(&ctx->arena, SYNQ_DOC_LINE_SUFFIX,
                                 sizeof(SynqDocLineSuffix));
  SynqDocLineSuffix* node =
      &((SynqDoc*)(ctx->arena.data + ctx->arena.offsets[id]))->line_suffix;
  node->child = child;
  return id;
}

uint32_t synq_doc_break_parent(SynqDocContext* ctx) {
  return synq_arena_alloc(&ctx->arena, SYNQ_DOC_BREAK_PARENT,
                          sizeof(SynqDocLeaf));
}

void synq_doc_debug_print(SynqDocContext* ctx,
                          uint32_t doc_id,
                          FILE* out,
                          int depth) {
...
char* synq_doc_layout(SynqDocContext* ctx,
                      uint32_t root_id,
  ...
  while (stack.count > 0) {
    ...
    switch (doc->tag) {
      ...
      case SYNQ_DOC_LINE: {
        if (mode == LAYOUT_MODE_FLAT) {
          ...
          for (uint32_t s = 0; s < pending_suffixes.count; s++) {
            LayoutEntry se = {indent, LAYOUT_MODE_FLAT,
                              pending_suffixes.data[s]};
      ...
      case SYNQ_DOC_BREAK_PARENT: {
        // No output; forces group to break via measure_flat returning
        // UINT32_MAX
        break;
      }
      ...
      case SYNQ_DOC_GROUP: {
        if (mode == LAYOUT_MODE_FLAT) {
          ...
          LayoutMode child_mode =
              (flat_width != UINT32_MAX && column + flat_width <= target_width)
                  ? LAYOUT_MODE_FLAT
                  : LAYOUT_MODE_BREAK;
  ...
  // Flush any remaining pending suffixes (e.g. trailing comment at end of
  // input)
  if (pending_suffixes.count > 0) {
