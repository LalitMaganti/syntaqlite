...
#define FOP_KW(s) {.kind = SYNQ_FOP_KW, .data = (s)}
...
#define FOP_CHILD_ITEM {.kind = SYNQ_FOP_CHILD_ITEM}

#define FOP_LINE {.kind = SYNQ_FOP_LINE}

#define FOP_SOFTLINE {.kind = SYNQ_FOP_SOFTLINE}

#define FOP_HARDLINE {.kind = SYNQ_FOP_HARDLINE}
...
#define FOP_CLAUSE(T, f, kw)                   \
  {.kind = SYNQ_FOP_CLAUSE,                    \
   .field_offset = offsetof(Syntaqlite##T, f), \
   .data = (kw)}
...
#define FOP_GROUP {.kind = SYNQ_FOP_GROUP}

#define FOP_NEST {.kind = SYNQ_FOP_NEST}

// --- Sequence (count children follow) ---

#define FOP_SEQ(n) {.kind = SYNQ_FOP_SEQ, .count = (n)}
...
#define FOP_SWITCH_DEFAULT(T, f, n)            \
  {.kind = SYNQ_FOP_SWITCH,                    \
   .field_offset = offsetof(Syntaqlite##T, f), \
   .count = (n),                               \
   .flags = SYNQ_FOP_HAS_DEFAULT}

#define FOP_CASE(v) {.kind = SYNQ_FOP_CASE, .enum_value = (v)}
...
#define FOP_FOR_EACH {.kind = SYNQ_FOP_FOR_EACH}
...
uint32_t synq_fmt_interpret_ops(SynqFmtCtx* ctx,
                                const void* node_ptr,
                                const SynqFmtOp* ops);
...
// Generated table mapping node tags to format recipes (indexed by
// SyntaqliteNodeTag). NULL entries mean the node type has no formatter.
