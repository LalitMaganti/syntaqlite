...
class FmtInterpretTest : public ::testing::Test {
 protected:
  ...
  void TearDown() override {
    if (initialized_)
      synq_ast_context_cleanup(&ast_ctx_);
  }

  bool initialized_ = false;

  // Run the interpreter on a node and return the formatted string.
  std::string Interpret(uint32_t node_id,
                        const SynqFmtOp* ops,
                        uint32_t width = 80) {
...
TEST_F(FmtInterpretTest, Literal) {
  Init("42");
  uint32_t id =
      synq_ast_literal(&ast_ctx_, SYNTAQLITE_LITERAL_TYPE_INTEGER, {0, 2});
...
TEST_F(FmtInterpretTest, LiteralString) {
  Init("'hello'");
  uint32_t id =
      synq_ast_literal(&ast_ctx_, SYNTAQLITE_LITERAL_TYPE_STRING, {0, 7});
  EXPECT_EQ(Interpret(id, literal_fmt), "'hello'");
}

// ============ Test: LimitClause ============
// Python DSL:
//   fmt=seq(child("limit"), if_set("offset", seq(kw(" OFFSET "),
//   child("offset"))))
...
TEST_F(FmtInterpretTest, LimitClauseWithOffset) {
  ...
  uint32_t limit =
      synq_ast_literal(&ast_ctx_, SYNTAQLITE_LITERAL_TYPE_INTEGER, {0, 2});
  uint32_t offset =
      synq_ast_literal(&ast_ctx_, SYNTAQLITE_LITERAL_TYPE_INTEGER, {10, 1});
...
TEST_F(FmtInterpretTest, OrderingTermDefault) {
  Init("col1");
  uint32_t col =
      synq_ast_column_ref(&ast_ctx_, {0, 4}, SYNQ_NO_SPAN, SYNQ_NO_SPAN);
  uint32_t id = synq_ast_ordering_term(
      &ast_ctx_, col, SYNTAQLITE_SORT_ORDER_ASC, SYNTAQLITE_NULLS_ORDER_NONE);
...
TEST_F(FmtInterpretTest, OrderingTermDescNullsFirst) {
  ...
  uint32_t col =
      synq_ast_column_ref(&ast_ctx_, {0, 4}, SYNQ_NO_SPAN, SYNQ_NO_SPAN);
  uint32_t id = synq_ast_ordering_term(
      &ast_ctx_, col, SYNTAQLITE_SORT_ORDER_DESC, SYNTAQLITE_NULLS_ORDER_FIRST);
...
TEST_F(FmtInterpretTest, OrderingTermAscNullsLast) {
  ...
  uint32_t col =
      synq_ast_column_ref(&ast_ctx_, {0, 4}, SYNQ_NO_SPAN, SYNQ_NO_SPAN);
  uint32_t id = synq_ast_ordering_term(
      &ast_ctx_, col, SYNTAQLITE_SORT_ORDER_ASC, SYNTAQLITE_NULLS_ORDER_LAST);
...
TEST_F(FmtInterpretTest, UnaryMinus) {
  Init("-42");
  uint32_t lit =
      synq_ast_literal(&ast_ctx_, SYNTAQLITE_LITERAL_TYPE_INTEGER, {1, 2});
...
TEST_F(FmtInterpretTest, UnaryNot) {
  Init("NOT true");
  uint32_t lit =
      synq_ast_literal(&ast_ctx_, SYNTAQLITE_LITERAL_TYPE_INTEGER, {4, 4});
...
TEST_F(FmtInterpretTest, BinaryPlusDefault) {
  Init("1 + 2");
  uint32_t left =
      synq_ast_literal(&ast_ctx_, SYNTAQLITE_LITERAL_TYPE_INTEGER, {0, 1});
  uint32_t right =
      synq_ast_literal(&ast_ctx_, SYNTAQLITE_LITERAL_TYPE_INTEGER, {4, 1});
  uint32_t id =
      synq_ast_binary_expr(&ast_ctx_, SYNTAQLITE_BINARY_OP_PLUS, left, right);
...
TEST_F(FmtInterpretTest, BinaryAnd) {
  ...
  uint32_t left =
      synq_ast_literal(&ast_ctx_, SYNTAQLITE_LITERAL_TYPE_INTEGER, {0, 1});
  uint32_t right =
      synq_ast_literal(&ast_ctx_, SYNTAQLITE_LITERAL_TYPE_INTEGER, {6, 1});
  uint32_t id =
      synq_ast_binary_expr(&ast_ctx_, SYNTAQLITE_BINARY_OP_AND, left, right);
...
TEST_F(FmtInterpretTest, ResultColumnExprNoAlias) {
  ...
  uint32_t lit =
      synq_ast_literal(&ast_ctx_, SYNTAQLITE_LITERAL_TYPE_INTEGER, {0, 2});
  uint32_t id = synq_ast_result_column(&ast_ctx_, no_flag, SYNQ_NO_SPAN, lit);
...
static const SynqFmtOp comma_list_fmt[] = {
    FOP_GROUP,  FOP_FOR_EACH_SEP, FOP_CHILD_ITEM,
    FOP_SEQ(2), FOP_KW(","),      FOP_LINE,
};
...
TEST_F(FmtInterpretTest, ForEachCommaList) {
  ...
  uint32_t a =
      synq_ast_literal(&ast_ctx_, SYNTAQLITE_LITERAL_TYPE_INTEGER, {0, 1});
  uint32_t b =
      synq_ast_literal(&ast_ctx_, SYNTAQLITE_LITERAL_TYPE_INTEGER, {3, 1});
  uint32_t c =
      synq_ast_literal(&ast_ctx_, SYNTAQLITE_LITERAL_TYPE_INTEGER, {6, 1});
...
TEST_F(FmtInterpretTest, ForEachNoSeparator) {
  ...
  uint32_t a =
      synq_ast_literal(&ast_ctx_, SYNTAQLITE_LITERAL_TYPE_INTEGER, {0, 1});
  uint32_t b =
      synq_ast_literal(&ast_ctx_, SYNTAQLITE_LITERAL_TYPE_INTEGER, {1, 1});
  uint32_t c =
      synq_ast_literal(&ast_ctx_, SYNTAQLITE_LITERAL_TYPE_INTEGER, {2, 1});
...
static const SynqFmtOp grouped_expr_fmt[] = {
    FOP_GROUP,    FOP_NEST,     FOP_SEQ(5),
    FOP_KW("("),  FOP_SOFTLINE, FOP_CHILD(SubqueryExpr, select),
    FOP_SOFTLINE, FOP_KW(")"),
};
...
TEST_F(FmtInterpretTest, GroupNestSoftline) {
  ...
  uint32_t inner =
      synq_ast_literal(&ast_ctx_, SYNTAQLITE_LITERAL_TYPE_INTEGER, {1, 5});
...
TEST_F(FmtInterpretTest, ClausePresent) {
  Init("SELECT * WHERE 1");
  uint32_t cond =
      synq_ast_literal(&ast_ctx_, SYNTAQLITE_LITERAL_TYPE_INTEGER, {15, 1});
  // Using DeleteStmt since it has a 'where' field
  uint32_t table =
      synq_ast_literal(&ast_ctx_, SYNTAQLITE_LITERAL_TYPE_INTEGER, {7, 1});
