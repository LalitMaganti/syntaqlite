...
extern inline void synq_ast_set_range(SynqAstContext* ctx,
                                      uint32_t node_id,
                                      uint32_t first,
                                      uint32_t last);
extern inline SynqSourceRange synq_ast_get_range(SynqAstContext* ctx,
                                                 uint32_t node_id);
extern inline void synq_ast_range_union(SynqAstContext* ctx,
                                        SynqSourceRange* acc,
                                        uint32_t child_id);
extern inline void synq_ast_range_union_span(SynqSourceRange* acc,
...
void synq_ast_list_flush(SynqAstContext* ctx) {
  if (ctx->list_acc_node_id == SYNTAQLITE_NULL_NODE)
    return;
...
uint32_t synq_ast_list_start(SynqAstContext* ctx,
                             uint8_t tag,
...
void synq_ast_print_source_span(FILE* out,
                                const char* source,
                                SyntaqliteSourceSpan span) {
...
SyntaqliteSourceSpan synq_span(SynqParseContext* ctx, SynqToken tok) {
  return (SyntaqliteSourceSpan){(uint32_t)(tok.z - ctx->zSql), (uint16_t)tok.n};
}
