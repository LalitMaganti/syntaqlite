...
// Grammar stack value: column name + type token (threaded through columnname
// rule)
...
// Grammar stack value: WITH clause (carries recursive flag without a wrapper
// node)
...
inline void synq_ast_set_range(SynqAstContext* ctx,
                               uint32_t node_id,
                               uint32_t first,
                               uint32_t last) {
...
inline void synq_ast_range_union(SynqAstContext* ctx,
  ...
  if (child.first == 0 && child.last == 0)
    return;
  if (child.first < acc->first)
    acc->first = child.first;
  if (child.last > acc->last)
    acc->last = child.last;
}
...
inline void synq_ast_range_union_span(SynqSourceRange* acc,
  ...
  if (span.length == 0)
    return;
  if (span.offset < acc->first)
    acc->first = span.offset;
