...
- `DialectEnv<'d>` parameterized on lifetime, with no `'static` requirement, enables stack-allocated test dialects
- Unsafe minimized: `DialectEnv` safe accessors wrap all raw pointer access; `Interpreter` has zero unsafe
- AST dump is in C (`syntaqlite_dump_node`), not Rust, so metadata has a single source of truth
...
Several files are **autogenerated and must never be edited directly**. They are regenerated by `tools/run-codegen` from `.synq` definitions and SQLite grammar. **The canonical list of generated files lives in `tools/generated-files.txt`**: always consult it, not this section, if you need the authoritative enumeration.
...
| `reference/` | Reference | Lookup documentation for flags, API surfaces, and schemas | "What are the options for X?" |
...
Section index pages (`_index.md`) contain a sentence or two describing the bucket, plus `sort_by = "weight"`.

### Cross-linking

Treat the docs as a web and **cross-link ruthlessly** using Zola's `@/` path syntax:
...
count. This is the validator-improvement worklist, sorted so that you can start at the top.
